...
yum install -q -y --skip-broken vnstat  
systemctl enable --now vnstat

yum install -q -y --skip-broken net-tools wget curl git nail e2fsprogs redhat-lsb neovim wget unzip

rm -Rf /tmp/dotfiles

timedatectl set-timezone America/New_York

yum install -q -y --skip-broken cronie-noanacron
...
yum install -q -y $(cat /tmp/rpms-default.txt /tmp/apache.txt /tmp/nginx.txt) --skip-broken
